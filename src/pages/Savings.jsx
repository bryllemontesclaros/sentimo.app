...
export default function Savings({ user, data }) {
  ...
  async function handleAdd() {
    ...
    await fsAdd(user.uid, 'goals', { name: form.name, target: parseFloat(form.target), current: parseFloat(form.current) || 0, date: form.date })
    ...
    <div className={styles.page}>
      ...
      <div className={styles.formCard}>
        ...
        <div className={`${styles.formRow} ${styles.col3}`}>
          ...
          <div className={styles.formGroup}><label>Target amount (₱)</label><input type="number" placeholder="0.00" value={form.target} onChange={e => set('target', e.target.value)} /></div>
        ...
        <div className={`${styles.formRow} ${styles.col2}`}>
          <div className={styles.formGroup}><label>Current saved (₱)</label><input type="number" placeholder="0.00" value={form.current} onChange={e => set('current', e.target.value)} /></div>
          ...
          return (
            <div key={g._id} className={styles.goalCard}>
              ...
              <div className={styles.goalMeta}>
                <span className={styles.goalSaved}>{fmt(g.current || 0)} saved</span>
                <span>of {fmt(g.target)}</span>
                {g.date && <span>{g.date}</span>}
              </div>
              <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                <input
                  type="number" placeholder="Add contribution"
                  value={contribs[g._id] || ''}
                  onChange={e => setContribs(c => ({ ...c, [g._id]: e.target.value }))}
                  style={{ flex: 1, padding: '7px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', fontSize: 12, outline: 'none' }}
                />
                <button onClick={() => handleContrib(g)} style={{ padding: '7px 14px', background: 'var(--accent)', color: '#0a0a0f', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Add</button>
              </div>
            </div>
          )
        })}
