...
export function generateMonthlyReport(data, profile, year, month, symbol) {
  ...
  const income = data.income.filter(t => t.date?.startsWith(ym))
  const expenses = data.expenses.filter(t => t.date?.startsWith(ym))
...
<head>
<meta charset="UTF-8">
<title>Sentimo Report — ${monthLabel}</title>
<style>
...
<div class="footer">Generated by Sentimo · Bawat piso, sinusubaybayan.</div>
</body>
</html>`
